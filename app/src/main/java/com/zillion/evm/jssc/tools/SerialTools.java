...
import com.serialport.funtionpage.KeyBoradActivity;
import com.serialport.funtionpage.MainMenu;
...
import com.zillion.evm.jssc.config.Constant;
...
public class SerialTools {
    ...
    Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 1:
                    // MC_CombinationPickDetailActivity
                    if (SerialTools.this.toolsListener != null && (SerialTools.this.toolsListener instanceof KeyBoradActivity)) {
                ...
                case 2:
                    ...
                    // MC_CombinationPickDetailActivity
                    if (SerialTools.this.toolsListener != null && (SerialTools.this.toolsListener instanceof MainMenu)) {
                        SerialTools.this.toolsListener.serialReturn((String) msg.obj, msg.what, SerialTools.this.userInfo);
                        return;
                    } else if (SerialTools.this.toolsListener != null) {
                        SerialTools.this.toolsListener.serialReturn((String) msg.obj, msg.what);
    ...
    public void openStore(int a, int b, int c) {
        ...
        try {
            ...
            Log.e(TAG, new StringBuilder(String.valueOf(a)).append(" ").append(b).append(" ").append(c).append(e2.getMessage()).toString(), e2);
            e2.printStackTrace();
        }
    }

    public void checkStore(int a, int b) {
        Log.i(TAG, "检查格子机状态： " + a + b);
        try {
    ...
    public void checkVender(int a, int b, Object mUserInfo) {
        Log.i(TAG, "checkVender mUserInfo:" + a + "," + b);
        this.userInfo = mUserInfo;
        try {
            if (this.mVender.isOpened() || this.mVender.openPort()) {
                try {
